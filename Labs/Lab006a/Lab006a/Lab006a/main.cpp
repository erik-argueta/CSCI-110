...
//  Ch8-1-2
//
//  Created by Erik Argueta on 2/10/20.
...
{
	ifstream in_file;
	in_file.open("/Users/erik-argueta/Documents/GitHub/CSCI-110/Labs/Lab006a/babynames.txt");
	if (in_file.fail()) { return 1; } // Check for failure after opening
	
	ofstream out_file;
	out_file.open("/Users/erik-argueta/Documents/GitHub/CSCI-110/Labs/Lab006a/OUTPUT.txt");
	
	float boy_total = 50;
	float girl_total = 50;
	
	cout << "Processing \"babynames.txt\"..." << endl << endl;
	cout << "Processing 153 Lines..." << endl << endl;
	
	while (boy_total > 0 || girl_total > 0) // To process each line
	{
		int rank;
		in_file >> rank;
		if (in_file.fail()) { return 2; }
